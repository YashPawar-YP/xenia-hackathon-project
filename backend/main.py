...
def is_admin(user_id: int, db: Session):
    ...
    if not user or user.role != "club_admin":  # Only club_admin now
...
def register(user: UserCreate, db: Session = Depends(get_db)):
    ...
    # Force everyone to register as student
    new_user = User(
        name=user.name,
        email=user.email,
...
def create_club(club: ClubCreate, db: Session = Depends(get_db)):
    existing_club = db.query(Club).filter(Club.admin_id == club.admin_id).first()
    if existing_club:
        raise HTTPException(status_code=400, detail="User already admins a club")
    ...
    # Auto-promote to club_admin when they create their first club
    ...
    return {
        ...
        "club_id": new_club.id,
        "admin_promoted": admin_user.role == "club_admin"
    }
...
def leave_club(club_id: int, user_id: int, db: Session = Depends(get_db)):
    ...
    if not club:
        raise HTTPException(status_code=404, detail="Club not found")
>>>>>>> cdab15faf83772e94ea3e37efc41e5b755f32bf5
...
def admin_dashboard(admin_id: int, db: Session = Depends(get_db)):
    ...
    # Only show clubs this admin manages
    clubs = db.query(Club).filter(Club.admin_id == admin_id).all()
